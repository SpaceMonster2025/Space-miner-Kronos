...
export const GameCanvas: React.FC<GameCanvasProps> = ({ gameState, onDock, onGameOver, playerState: initialPlayerState }) => {
  ...
  const outpostsRef = useRef<{x: number, y: number}[]>([]);
  ...
  useEffect(() => {
    ...
    // Generate Stars (Increased count for larger world)
    const stars = [];
    ...
    // Generate Outposts
    const outposts = [];
    for(let i=0; i<OUTPOST_CONFIG.COUNT; i++) {
    ...
    outpostsRef.current = outposts;

    // Generate Asteroids (Increased count)
  ...
  // --- Outpost Actions ---
  const handleOutpostRefuel = () => {
    if (!activeOutpost) return;
    const ship = shipRef.current;
    // Surge pricing! 2x to 5x normal cost (0.1 base)
    const surge = 2 + Math.floor(Math.random() * 4);
    const costPerUnit = 0.1 * surge;
    const needed = ship.shipConfig.maxFuel - ship.currentFuel;
    
    if (needed <= 0) return; // Full
    
    const totalCost = Math.ceil(needed * costPerUnit);
    
    if (ship.credits >= totalCost) {
      ship.credits -= totalCost;
      ship.currentFuel = ship.shipConfig.maxFuel;
      // Add particles/feedback
      particlesRef.current.push({
        x: ship.position.x, y: ship.position.y - 30,
        vx: 0, vy: -1, life: 60, maxLife: 60,
        color: '#f59e0b', size: 0, text: `REFUELED -${totalCost}cr`
      });
    } else {
      // Partial
      const affordable = Math.floor(ship.credits / costPerUnit);
      if (affordable > 0) {
        ship.credits = 0;
        ship.currentFuel += affordable;
        particlesRef.current.push({
          x: ship.position.x, y: ship.position.y - 30,
          vx: 0, vy: -1, life: 60, maxLife: 60,
          color: '#f59e0b', size: 0, text: `PARTIAL REFUEL`
        });
      } else {
        // Too poor
        particlesRef.current.push({
          x: ship.position.x, y: ship.position.y - 30,
          vx: 0, vy: -1, life: 60, maxLife: 60,
          color: '#ef4444', size: 0, text: `INSUFFICIENT FUNDS`
        });
      }
    }
  };

  const handleOutpostTow = () => {
    if (!activeOutpost) return;
    const ship = shipRef.current;
    // Base cost + distance fee
    const dist = Math.sqrt(ship.position.x**2 + ship.position.y**2);
    const cost = Math.ceil(OUTPOST_CONFIG.TOW_COST_BASE + (dist * 0.05));
    
    if (ship.credits >= cost) {
      ship.credits -= cost;
      onDock(ship); // Instant dock
    } else {
       particlesRef.current.push({
          x: ship.position.x, y: ship.position.y - 30,
          vx: 0, vy: -1, life: 60, maxLife: 60,
          color: '#ef4444', size: 0, text: `NEED ${cost}cr FOR TOW`
        });
    }
  };
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Reverse (S/Down)
      const isReversing = keysRef.current['KeyS'] || keysRef.current['ArrowDown'];

      if ((isThrusting || isReversing) && ship.currentFuel > 0) {
        // Direction logic
        const direction = isReversing ? -0.5 : 1; // Reverse is slower
        const thrustPower = ship.shipConfig.acceleration * direction;
        
        ship.velocity.x += Math.cos(ship.rotation) * thrustPower;
        ship.velocity.y += Math.sin(ship.rotation) * thrustPower;
        
        // Fuel burn (reverse burns less?)
        ship.currentFuel -= ship.shipConfig.thrustConsumptionRate * (isReversing ? 0.6 : 1.0);
        ...
        if (isThrusting) {
            ...
            // Retro Rockets (Front/Nose) - firing forward to push back
            const noseOffset = 20;
            const rx = ship.position.x + Math.cos(ship.rotation) * noseOffset;
            const ry = ship.position.y + Math.sin(ship.rotation) * noseOffset;
            
            // Two streams from nose area
            particlesRef.current.push({
                x: rx + Math.cos(ship.rotation + Math.PI/2) * 4,
                y: ry + Math.sin(ship.rotation + Math.PI/2) * 4,
                vx: ship.velocity.x + Math.cos(ship.rotation) * 4 + (Math.random()-0.5),
                vy: ship.velocity.y + Math.sin(ship.rotation) * 4 + (Math.random()-0.5),
                life: 15,
                maxLife: 15,
                color: '#a3a3a3', // Grey/White gas
                size: 1.5
            });
            particlesRef.current.push({
                x: rx + Math.cos(ship.rotation - Math.PI/2) * 4,
      ...
      if (!alienRef.current) {
          if (Math.random() < ALIEN_CONFIG.SPAWN_RATE) {
              const angle = Math.random() * Math.PI * 2;
              const dist = Math.max(width, height) * 1.2; // Spawn outside view relative to ship (screen based)
              alienRef.current = {
                  id: `alien-${Date.now()}`,
      ...
      if (alienRef.current) {
          ...
          // AI Logic with Fleeing Priority
          if (alien.state === 'FLEEING') {
             // Fleeing Movement: Run away from player
             const angle = Math.atan2(dy, dx);
             // Inverse thrust (move away)
             alien.vx -= Math.cos(angle) * 0.15;
             alien.vy -= Math.sin(angle) * 0.15;
             
             // Escape check (Despawn if too far)
             if (dist > 2500) {
                 particlesRef.current.push({
                     x: ship.position.x, y: ship.position.y - 50,
                     vx: 0, vy: -0.5, life: 120, maxLife: 120,
                     color: '#ef4444', size: 0, text: "ALIEN ESCAPED WITH CARGO!"
                 });
                 alienRef.current = null; // Despawn
                 soundManagerRef.current?.stopAlienHum();
             }

          } else {
              // Normal Behavior: Chase or Drain
          ...
          // Apply velocity if alien still exists
          if (alienRef.current) {
              // Speed Cap
              // Fleers can go a bit faster to be annoying
              const maxSpeed = alien.state === 'FLEEING' ? ALIEN_CONFIG.SPEED * 1.2 : ALIEN_CONFIG.SPEED;
              const alienSpeed = Math.sqrt(alien.vx**2 + alien.vy**2);
              if (alienSpeed > maxSpeed) {
                  alien.vx = (alien.vx / alienSpeed) * maxSpeed;
                  alien.vy = (alien.vy / alienSpeed) * maxSpeed;
              }
              ...
              if (alien.state === 'DRAINING' && dist < ALIEN_CONFIG.DRAIN_RANGE) {
                  ...
                  if (alien.drainTimer >= ALIEN_CONFIG.DRAIN_INTERVAL) {
                      ...
                      if (availableTypes.length > 0) {
                          ...
                          // Check for flee threshold (10% of MAX cargo capacity)
                          const threshold = Math.ceil(ship.shipConfig.maxCargo * 0.1);
                          if (alien.totalStolenCount >= threshold) {
                              alien.state = 'FLEEING';
                              particlesRef.current.push({
                                  x: alien.x,
                                  y: alien.y - 30,
      ...
      if (mouseRef.current.isDown && ship.currentFuel > 0) {
          ...
          if (!attackingAlien) {
            for (let ast of asteroidsRef.current) {
                // Optimization: Only check nearby asteroids for collision
                if (Math.abs(ast.x - worldMouseX) > 100 || Math.abs(ast.y - worldMouseY) > 100) continue;
      ...
      // --- Outpost Logic ---
      let foundOutpost = null;
      for(const op of outpostsRef.current) {
         const dx = op.x - ship.position.x;
         const dy = op.y - ship.position.y;
         const dist = Math.sqrt(dx*dx + dy*dy);
         if (dist < OUTPOST_CONFIG.RADIUS * 1.5) {
             // Stopped check
             if (speed < 0.5) {
                 foundOutpost = { x: op.x, y: op.y, distance: dist };
             }
         }
      }
      setActiveOutpost(foundOutpost);


      // Docking Logic (Main Station)
      ...
      nebulasRef.current.forEach(neb => {
        ...
        // Culling for massive performance gain
        if (px + neb.size < viewX - cullMargin || px - neb.size > viewX + viewW + cullMargin || 
            py + neb.size < viewY - cullMargin || py - neb.size > viewY + viewH + cullMargin) return;
      ...
      galaxiesRef.current.forEach(gal => {
        ...
        if (px + gal.size < viewX - cullMargin || px - gal.size > viewX + viewW + cullMargin || 
            py + gal.size < viewY - cullMargin || py - gal.size > viewY + viewH + cullMargin) return;
      ...
      starsRef.current.forEach(star => {
          ...
          if (px < viewX - 10 || px > viewX + viewW + 10 || 
              py < viewY - 10 || py > viewY + viewH + 10) return;
      ...
      if (STATION_POSITION.x > viewX - STATION_RADIUS && STATION_POSITION.x < viewX + viewW + STATION_RADIUS &&
          STATION_POSITION.y > viewY - STATION_RADIUS && STATION_POSITION.y < viewY + viewH + STATION_RADIUS) {
      ...
      // Draw Outposts
      outpostsRef.current.forEach((op, idx) => {
          // Culling
      ...
      asteroidsRef.current.forEach(ast => {
          // Culling
          if (ast.x + ast.radius < viewX - 100 || ast.x - ast.radius > viewX + viewW + 100 ||
              ast.y + ast.radius < viewY - 100 || ast.y - ast.radius > viewY + viewH + 100) return;
      ...
      if (alienRef.current) {
          const alien = alienRef.current;
          // Culling for alien? Usually always render if exists as it chases, but good for safety
          if (alien.x + 50 > viewX && alien.x - 50 < viewX + viewW && alien.y + 50 > viewY && alien.y - 50 < viewY + viewH) {
            ctx.save();
      ...
      particlesRef.current.forEach((p, i) => {
          if (p.x < viewX - 20 || p.x > viewX + viewW + 20 || p.y < viewY - 20 || p.y > viewY + viewH + 20) return;
          p.x += p.vx;
          p.y += p.vy;
          p.life--;
      ...
      // Radar Blips
      const radarScale = radarSize / 2000; // Keep radar short range
      asteroidsRef.current.forEach(ast => {
          // Optimization: Simple dist check first
          if (Math.abs(ast.x - ship.position.x) > 2000 || Math.abs(ast.y - ship.position.y) > 2000) return;
  ...
  return (
    <>
      ...
      {/* Outpost UI Overlay */}
      {activeOutpost && (
          <div className="absolute top-20 left-1/2 -translate-x-1/2 bg-black/80 border border-amber-500 p-4 text-amber-500 font-mono text-center rounded backdrop-blur-sm z-30">
              <h2 className="text-lg font-bold mb-2">EMERGENCY OUTPOST</h2>
              <p className="text-xs mb-4 text-amber-300">SURGE PRICING ACTIVE</p>
              <div className="flex gap-4">
                  <button 
                    onClick={handleOutpostRefuel}
                    className="border border-amber-500 px-4 py-2 hover:bg-amber-500 hover:text-black transition-colors"
                  >
                      REFUEL (HIGH COST)
                  </button>
                  <button 
                    onClick={handleOutpostTow}
                    className="border border-amber-500 px-4 py-2 hover:bg-amber-500 hover:text-black transition-colors"
                  >
                      REQ TOW ({Math.ceil(OUTPOST_CONFIG.TOW_COST_BASE + (activeOutpost.distance * 0.05))}cr)
                  </button>
              </div>
          </div>
      )}
